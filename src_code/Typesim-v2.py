...
def create_tray_icon():
    ...
    menu = (item('Restore', restore_from_tray), item('Exit', lambda: root.destroy()))
    tray_icon = Icon("TypeSim", image, "TypeSim - Running", menu)
...
def start_typing():
    ...
    try:
        ...
        messagebox.showerror("Error", f"An error occurred during typing:\n{str(e)}")
    finally:
        if tray_icon:
            tray_icon.stop()
        root.deiconify()
...
def on_enter(e):
    github_link.configure(foreground="#3939E1")
def on_leave(e):
    if mode_switch.instate(["selected"]):
        github_link.configure(foreground="#3498db") 
    else:
        github_link.configure(foreground="#0066cc")
